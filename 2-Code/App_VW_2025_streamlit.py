...
async def streamlit_interface():
    # Configuration de la page Streamlit avec des paramètres de design
    st.set_page_config(
        page_title="Assistant Météo Vocal",
        page_icon=":partly_sunny:",
        layout="wide"
    )
    
    # Insertion d'un style CSS personnalisé pour améliorer l'aspect visuel
    st.markdown(
        """
        <style>
        body {
    ...
    st.markdown("### Obtenez les prévisions météorologiques grâce à la reconnaissance vocale")
    
    # Barre latérale pour les instructions
    st.sidebar.title("Menu")
    st.sidebar.info(
        "Cliquez sur le bouton **Parlez** pour activer la reconnaissance vocale et obtenir les prévisions météo.\n\n"
        "Assurez-vous que votre microphone est correctement configuré."
    )
    
    # Optionnel : affichage d'une image/banner si disponible
    # st.image("https://example.com/votre_image.png", use_column_width=True)
    ...
    if st.button("Parlez"):
        ...
        if text:
            ...
            try:
                ...
                st.success(f"Prévisions météo pour **{location}** ({date}):")
                st.markdown(f"**Description :** {meteo_data['description']}")
                st.markdown(f"**Température :** {meteo_data['temperature']}°C")
                st.markdown(f"**Humidité :** {meteo_data['humidity']}%")
                st.markdown(f"**Vent :** {meteo_data['wind_speed']} km/h")
